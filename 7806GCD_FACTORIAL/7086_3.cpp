...
n! % k =( (n%k) * (n-1!%k) ) % k
 ...
 7086_1 : 메모리 초과로 틀렸다고 나옴 ㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠㅠ

 7086_2 : boolean 지웠고 깔끔하게 만들었지만 recursion을 쓰면 안되는 듯. 여전히 메모리 초과
 7086_3 : recursive말고 for문을 사용해서 풀어보자 -> 시간 초과
*/
...
int factMoulder(int a, int b) {
	...
	int result = 1; //3!%10 풀어서 쭉 써보면 result가 왜 1로 시작하는지 알 수 있음.
	for (int i = 1; i < a + 1; i++) {
		result = ((i%b) * result ) % b;
	}
	return result;
